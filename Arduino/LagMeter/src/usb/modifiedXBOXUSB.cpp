...
#include "modifiedXBOXUSB.h"
// To enable serial debugging see "settings.h"
//#define EXTRADEBUG // Uncomment to get even more debugging data
//#define PRINTREPORT // Uncomment to print the report send by the Xbox 360 Controller

ModifiedXBOXUSB::ModifiedXBOXUSB(USB *p) :
...
uint8_t ModifiedXBOXUSB::Init(uint8_t parent, uint8_t port, bool lowspeed) {
...
uint8_t ModifiedXBOXUSB::Release() {
...
void ModifiedXBOXUSB::readReport() {
        if(readBuf == NULL)
                return;
...
void ModifiedXBOXUSB::printReport() { //Uncomment "#define PRINTREPORT" to print the report send by the Xbox 360 Controller
#ifdef PRINTREPORT
        if(readBuf == NULL)
                return;
...
uint8_t ModifiedXBOXUSB::getButtonPress(ButtonEnum b) {
...
bool ModifiedXBOXUSB::getButtonClick(ButtonEnum b) {
        if(b == L2) {
                if(L2Clicked) {
                        L2Clicked = false;
                        return true;
...
int16_t ModifiedXBOXUSB::getAnalogHat(AnalogHatEnum a) {
        return hatValue[a];
}

/* Xbox Controller commands */
void ModifiedXBOXUSB::XboxCommand(uint8_t* data, uint16_t nbytes) {
...
void ModifiedXBOXUSB::setLedRaw(uint8_t value) {
        writeBuf[0] = 0x01;
        writeBuf[1] = 0x03;
        writeBuf[2] = value;
...
void ModifiedXBOXUSB::setLedOn(LEDEnum led) {
        if(led == OFF)
                setLedRaw(0);
...
void ModifiedXBOXUSB::setLedBlink(LEDEnum led) {
        setLedRaw(pgm_read_byte(&XBOX_LEDS[(uint8_t)led]));
}

void ModifiedXBOXUSB::setLedMode(LEDModeEnum ledMode) { // This function is used to do some special LED stuff the controller supports
        setLedRaw((uint8_t)ledMode);
}

void ModifiedXBOXUSB::setRumbleOn(uint8_t lValue, uint8_t rValue) {
        writeBuf[0] = 0x00;
        writeBuf[1] = 0x08;
        writeBuf[2] = 0x00;
