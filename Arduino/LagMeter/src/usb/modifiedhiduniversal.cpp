...
ModifiedHIDUniversal::ModifiedHIDUniversal(USB *p) :
USBHID(p),
qNextPollTime(0),
pollInterval(0),
bPollEnable(false),
...
uint16_t ModifiedHIDUniversal::GetHidClassDescrLen(uint8_t type, uint8_t num) {
...
void ModifiedHIDUniversal::Initialize() {
...
bool ModifiedHIDUniversal::SetReportParser(uint8_t id, HIDReportParser *prs) {
        for(uint8_t i = 0; i < MAX_REPORT_PARSERS; i++) {
...
HIDReportParser* ModifiedHIDUniversal::GetReportParser(uint8_t id) {
        if(!bHasReportId)
...
uint8_t ModifiedHIDUniversal::Init(uint8_t parent, uint8_t port, bool lowspeed) {
...
ModifiedHIDUniversal::HIDInterface* ModifiedHIDUniversal::FindInterface(uint8_t iface, uint8_t alt, uint8_t proto) {
...
void ModifiedHIDUniversal::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto, const USB_ENDPOINT_DESCRIPTOR *pep) {
        ...
        if(index) {
                ...
                // Set poll interval to LOWEST poll interval obtained from endpoints
                if(pollInterval == 0 || pollInterval > pep->bInterval)
                        pollInterval = pep->bInterval;
...
uint8_t ModifiedHIDUniversal::Release() {
...
bool ModifiedHIDUniversal::BuffersIdentical(uint8_t len, uint8_t *buf1, uint8_t *buf2) {
...
void ModifiedHIDUniversal::ZeroMemory(uint8_t len, uint8_t *buf) {
        for(uint8_t i = 0; i < len; i++)
                buf[i] = 0;
}

void ModifiedHIDUniversal::SaveBuffer(uint8_t len, uint8_t *src, uint8_t *dest) {
        for(uint8_t i = 0; i < len; i++)
                dest[i] = src[i];
}

uint8_t ModifiedHIDUniversal::Poll() {
        uint8_t rcode = 0;
...
uint8_t ModifiedHIDUniversal::SndRpt(uint16_t nbytes, uint8_t *dataptr) {
